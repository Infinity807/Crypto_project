...
impl<Stream: Read + Write> WebSocket<Stream> {
    ...
    pub fn read(&mut self, f: impl FnMut()) -> Result<Message> {
        self.context.read(&mut self.socket, f)
    }
...
impl WebSocketContext {
    ...
    pub fn read<Stream>(&mut self, stream: &mut Stream, mut f: impl FnMut()) -> Result<Message>
    where
        Stream: Read + Write,
    {
        ...
        loop {
            ...
            if let Some(message) = self.read_message_frame(stream, &mut f)? {
                trace!("Received message {message:?}");
                // println!("{:?}", message.clone().into_text());
        ...
        Stream: Read + Write,
    {
        ...
        match message {
            Message::Text(data) => {
                let frame = Frame::message(data.as_bytes(), OpCode::Data(OpData::Text), true);
            ...
            Message::Binary(data) => {
                let frame = Frame::message(&data, OpCode::Data(OpData::Binary), true);
                let should_flush = self._write(stream, Some(frame))?;
                if should_flush {
                    self.flush(stream)?;
                }
            ...
            Message::Pong(data) => {
                // self.set_additional(Frame::pong(data)); // TODO
                self.buffer_frame(stream, Frame::pong(&data))?;
            ...
            // Message::Frame(f) => f, // ASSUMPTION: We never write a frame directly to the websocket stream
        };
        Ok(())
    }
        ...
        Stream: Read + Write,
    {
        ...
        let should_flush = self.unflushed_additional;
        // Upon receipt of a Ping frame, an endpoint MUST send a Pong frame in
        // response, unless it already received a Close frame. It SHOULD
        // respond with Pong frame as soon as is practical. (RFC 6455)
        // let should_flush = if self.unflushed_additional {
        //     trace!("Sending pong/close");
            ...
            //     Err(Error::WriteBufferFull(Message::Frame(msg))) => {
                    ...
                    // self.set_additional(msg); // TODO: Kryptonite's Write Buffer will never be full (Ring Buffer)
        //             false
        //         }
        //         Err(err) => return Err(err),
        //         Ok(_) => true,
        ...
        Stream: Read + Write,
    {
        if let WebSocketState::Active = self.state {
            ...
            let close_payload = Frame::get_close_payload(&code);
            let frame = Frame::close(&close_payload);
    ...
    fn read_message_frame<Stream>(&mut self, stream: &mut Stream, f: impl FnMut()) -> Result<Option<Message>>
    where
        Stream: Read + Write,
    {
        // let payload = self
        //     .frame
        //     .read_frame(stream, self.config.max_frame_size, f)
        //     .check_connection_reset(self.state)?.unwrap().payload().to_vec();

        // self.buffer_frame(stream, Frame::pong(&payload));
            ...
            match self.role {
                Role::Server => {
                    if frame.is_masked() {
                        ...
                        // frame.apply_mask(); // ASSUMPTION: We never play the role of server.
                ...
                OpCode::Control(ctl) => {
                    match ctl {
                        ...
                        OpCtl::Close => {
                            ...
                            Ok(self.do_close(stream, close).map(Message::Close))
                        },
                        OpCtl::Reserved(i) => {
                ...
                OpCode::Data(data) => {
                    ...
                    match data {
                        ...
                        OpData::Text | OpData::Binary => {
                            // TODO: Ditch the UTF-8 validity checking assuming that the exchange server never sends
                            // ill-formed frames
                            let msg = {
                                ...
                                let mut m = IncompleteMessage::new(message_type); // TODO: `extend`?
    ...
    fn do_close<'t, Stream>(&mut self, stream: &mut Stream, close: Option<CloseFrame<'t>>) -> Option<Option<CloseFrame<'t>>>
    where
        Stream: Read + Write,
    {
        ...
        match self.state {
            WebSocketState::Active => {
                ...
                let close_payload = Frame::get_close_payload(&close);
                let reply = Frame::close(&close_payload);
                debug!("Replying to close with {reply:?}");
                // self.set_additional(reply); // TODO
                self.buffer_frame(stream, reply).unwrap(); // TODO: error handling
                self.unflushed_additional = true;
    ...
    // /// Replace `additional_send` if it is currently a `Pong` message. --> TODO: We do not do this check does it effect any case?
