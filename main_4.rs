...
mod json_bookticker;
mod json_trades;
use json_bookticker::{BookTicker,parse_book_ticker};
use json_orderbook::{OrderBook,parse_orderbook};
use json_trades::{Trade,parse_trade};
...
fn main()->Result<(), BinanceError> {
    ...
    const DEPTH: usize =5;
    let mut book = OrderBook::<DEPTH>::new();
    let mut ticker = BookTicker::new();
    ...
    loop{
        ...
        if let Ok(msg) = btcfdusd_orderbook_rx.try_recv(){
            ...
            parse_orderbook(msg.as_bytes(), &mut book);
        ...
        //     let start = Instant::now();
        //     parse_trade(msg.as_bytes(), &mut trade);            
        //     //let (bid, ask) = json_parse::process_bid_ask_jsonic(&msg);
        //     total_ns += start.elapsed().as_nanos();
        //     println!("{}", total_ns);
        //     println!("\nResults:");
        //     println!("Symbol: {}", std::str::from_utf8(&trade.symbol[..trade.symbol_len]).unwrap());
        //     println!("Price: {:.8}", trade.price);
        //     println!("Quantity: {:.8}", trade.quantity);
        //     println!("Timestamp: {}", trade.timestamp);
        //     println!("Is Maker: {}", trade.is_maker);
        //     println!("Average parsing bid: {}", msg);
        //     }
        // }
    ...
    //    if let Ok(msg) = rx.try_recv(){
        ...
        //     let process = SystemTime::now();
        //     if let Ok(dhurt) = process.duration_since(now) {
        //         // Successfully calculated the duration, print it
        //         println!("Time difference: {} seconds, {} nanoseconds", msg, dhurt.subsec_nanos());
        //     } else {
        //         // Error calculating the time difference
        //         println!("Error calculating the time difference");
        //     }
            //let process_now = process.duration_since(now);
            //println!("BTCFDUSD Bid: {}    BTCFDUSD Ask: {} msg:{}", bid, ask, msg);
            //process_market_data_and_orders(&mut client, bid, ask);
            // let yo = SystemTime::now();
            // if let Ok(dhurt) = yo.duration_since(now) {
            //     // Successfully calculated the duration, print it
            //     println!("Time difference: {} seconds, {} nanoseconds", dhurt.as_secs(), dhurt.subsec_nanos());
            // } else {
            //     // Error calculating the time difference
            //     println!("Error calculating the time difference");
            // }
